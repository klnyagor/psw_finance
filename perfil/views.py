...
def cadastrar_banco(request):
    ...
    # verificar se as variaveis estão vazias/em branco
    if not all([apelido.strip(), banco.strip(), tipo.strip(), valor.strip(), icone]):
...
def cadastrar_categoria(request):
    ...
    # validações
    if not nome.strip() or not isinstance(essencial, bool):
        messages.add_message(request, constants.ERROR, 'Preencha todos os campos corretamente!')
        return redirect('/perfil/gerenciar/')
